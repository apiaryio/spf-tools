...
function callSpfTools(cb) {
  ...
  child_process.exec(compare_command, options, (error, stdout, stderr) => {
    if (error) {
      ...
      if (error.code == 1) {
        var update_command = `despf.sh ${process.env.ORIG_SPF} | normalize.sh | simplify.sh | mkblocks.sh ${process.env.DOMAIN} | DOMAIN=${process.env.DOMAIN} ORIG_SPF=${process.env.ORIG_SPF} DRY_RUN=${process.env.DRY_RUN} node ./scripts/plugins/akamai/akamai.js`;
